...
def create_student():
    ...
    if not data:
        return ({'error': 'No fields provided in the request body'}), 400
    
    provided_fields = set(data.keys())

    invalid_fields = VALID_FIELDS - provided_fields

    if invalid_fields:
        return ({'error': 'Invalid fields provided in the request body'}), 400
...
def delete_student(student_id):
    ...
    return jsonify({'message': f'Student {deleted_student['first_name']} {deleted_student['last_name']} successfully was deleted'}), 204

@app.route('/students/<int:student_id>', methods=['PATCH'])
def patch_student_age(student_id):
    data = request.get_json()
    if not data:
        return jsonify({'error': 'No fileds provided in the request body'}), 400
    provided_fields = set(data.keys())
    allowed_patched_fields = {'age'}
    invalid_fields = provided_fields - allowed_patched_fields
    if invalid_fields:
        return jsonify({'eroor': 'Invalid fields in PATCH: {invalid_fields}. Only the age is allowed '}), 400
    if 'age' not in data:
        return jsonify({'error': 'Field AGE is required'}), 400
    students = read_students()
    index, student = find_student_by_id(students, student_id)
    if student is None:
        return jsonify({'error': 'Student with id {student_id} is not found'}), 404
...
def update_student(student_id):
    ...
    if invalid_fields:
        return jsonify({'eroor': 'Invalid fields in PUT: {invalid_fields}. Only the age is allowed '}), 400
    missing_fields = VALID_FIELDS - provided_fields
    if missing_fields:
...
if __name__ == "__main__":
    ...
    # (debug = False, port = 8080, host = '0.0.0.0', threaded = True)